...
from unum_ops.sparse_kernel_extension import get_block_table_ref_torch, get_block_table_ref_triton
...
def gen_topk(batch_size, topk_max_val, device, seed=42):
...
def generate_diverse_test_data(batch_size_val, seqlen_val, topk_val, device, seed_offset=0):
    ...
    topk_idx = torch.zeros((kHeadGroup, batch_size_val, kSparseTopK), dtype=torch.int32, device=device)
    topk_idx[:, :, :actual_topk] = topk_idx_sorted.to(device=device)

    gen2 = torch.Generator(device='cpu')
    gen2.manual_seed(local_seed + 10000)
    block_table = torch.randint(1, 10000, (batch_size_val, seqlen_val), dtype=torch.int32, generator=gen2).to(device=device)
    ...
    seqlen_q_blocks = torch.randint(max_blocks // 4, max_blocks + 1, (batch_size_val,), dtype=torch.int32, generator=gen3)
...
def diverse_test_data(request, device):
    ...
    return generate_diverse_test_data(params['batch'], params['seqlen'], params['topk'], device, params['seed'])
...
def test_bench_get_table(test_data):
    ...
    kernels = [
        ('Torch', lambda: get_block_table_ref_torch(topk_idx, block_table, token_to_bs, seqlen_q, seqlen_q)),
        ('Triton', lambda: get_block_table_ref_triton(topk_idx, block_table, token_to_bs, seqlen_q, seqlen_q)),
        ('CUDA v1', lambda: sparse_kernel_extension.get_block_table_v1(topk_idx, block_table, token_to_bs, seqlen_q, seqlen_q, kSparseTopK)),
        ('CUDA v2', lambda: sparse_kernel_extension.get_block_table_v2(topk_idx, block_table, token_to_bs, seqlen_q, seqlen_q, kSparseTopK)),
        ('CUDA v3', lambda: sparse_kernel_extension.get_block_table_v3(topk_idx, block_table, token_to_bs, seqlen_q, seqlen_q, kSparseTopK)),
    ]
    ...
    for name in ['Torch', 'Triton', 'CUDA v1', 'CUDA v2', 'CUDA v3']:
        ...
        data.append({'Kernel': name, 'Latency (ms)': f'{t:.3f}', 'vs CUDA v1': vs_v1, 'vs CUDA v3': vs_v3})
