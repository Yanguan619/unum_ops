...
template <HardEvent eventType> __aicore__ inline void SyncFor310P() {
    event_t eventId = static_cast<event_t>(GetTPipePtr()->FetchEventID(eventType));
    SetFlag<eventType>(eventId);
    WaitFlag<eventType>(eventId);
}

__aicore__ inline void DataCopyAtomicCompat(
    GlobalTensor<float> dstGm, LocalTensor<float> srcLocal, uint64_t elemOffset) {
    uint64_t alignedOffset = elemOffset / B32_DATA_NUM_PER_BLOCK * B32_DATA_NUM_PER_BLOCK;
    uint32_t lane = static_cast<uint32_t>(elemOffset - alignedOffset);

    SyncFor310P<HardEvent::V_S>();
    float value = srcLocal.GetValue(0);
    for (int32_t i = 0; i < B32_DATA_NUM_PER_BLOCK; ++i) {
        srcLocal.SetValue(i, 0.0f);
    }
    srcLocal.SetValue(lane, value);
    SyncFor310P<HardEvent::S_MTE3>();

    DataCopy(dstGm[alignedOffset], srcLocal, B32_DATA_NUM_PER_BLOCK);
}
#endif
...
#if defined(__CCE_AICORE__) && __CCE_AICORE__ == 200
        DataCopyAtomicCompat(gradDepthGm_, broadTmp[j * B32_DATA_NUM_PER_BLOCK], rd);
#else
