...
template <bool with_depth> class BEVPoolV3Kernel {
  public:
  ...
  private:
    ...
    __aicore__ inline void ProcessDualRank(
        int32_t rankOffset, uint32_t step0, uint32_t step1, uint64_t featOffset1, uint64_t depOffset1, int32_t idx);
...
__aicore__ inline void BEVPoolV3Kernel<with_depth>::CopyIn(
    uint8_t ping, uint32_t step, uint64_t off, uint64_t featOff, uint64_t depOff) {
    ...
    for (int32_t j = 0; j < step; j++) {
        ...
        DataCopyPad(
            depthTmp_[depOff + j * B32_DATA_NUM_PER_BLOCK], depthGm_[rd], {1, uint32_t(sizeof(float)), 0, 0, 0}, {});
    }
...
template <bool with_depth>
__aicore__ inline void BEVPoolV3Kernel<with_depth>::CopyOut(
...
template <bool with_depth>
__aicore__ inline void BEVPoolV3Kernel<with_depth>::ProcessDualRank(
    int32_t rankOffset, uint32_t step0, uint32_t step1, uint64_t featOffset1, uint64_t depOffset1, int32_t idx) {
...
template <bool with_depth>
__aicore__ inline void BEVPoolV3Kernel<with_depth>::ProcessSingle(
    ...
    if (ping == 0) {
        {
            ...
            DataCopyPad(ranksBev_[rankOffset0], ranksBevGm_[taskIdx * avgRankNum_],
                {1, uint32_t(actualRankNum * sizeof(int32_t)), 0, 0, 0}, {});
            DataCopyPad(ranksFeat_[rankOffset0], ranksFeatGm_[taskIdx * avgRankNum_],
                {1, uint32_t(actualRankNum * sizeof(int32_t)), 0, 0, 0}, {});
            ...
            DataCopyPad(ranksDepth_[rankOffset0], ranksDepthGm_[taskIdx * avgRankNum_],
                {1, uint32_t(actualRankNum * sizeof(int32_t)), 0, 0, 0}, {});
        }
        if (unlikely(taskIdx != taskEndIdx_ - 1)) {
            uint64_t taskIdx1 = taskIdx + 1;
            uint32_t actualRankNum1 = actualRankNum;
            if (taskIdx1 == totalTaskNum_ - 1) {
                actualRankNum1 = tailRankNum_;
            }
            WaitFlag<HardEvent::V_MTE2>(1);
            DataCopyPad(ranksBev_[rankOffset1], ranksBevGm_[taskIdx1 * avgRankNum_],
                {1, uint32_t(actualRankNum1 * sizeof(int32_t)), 0, 0, 0}, {});
            DataCopyPad(ranksFeat_[rankOffset1], ranksFeatGm_[taskIdx1 * avgRankNum_],
                {1, uint32_t(actualRankNum1 * sizeof(int32_t)), 0, 0, 0}, {});
            ...
            DataCopyPad(ranksDepth_[rankOffset1], ranksDepthGm_[taskIdx1 * avgRankNum_],
                {1, uint32_t(actualRankNum1 * sizeof(int32_t)), 0, 0, 0}, {});
        }
    }
...
template <bool with_depth> __aicore__ inline void BEVPoolV3Kernel<with_depth>::Process() {
    uint8_t ping = 0;
...
template <bool with_depth> __aicore__ inline void BEVPoolV3Kernel<with_depth>::ProcessWithoutDepth() {
    for (uint32_t i = taskStartIdx_; i < taskEndIdx_; ++i) {
        uint32_t actualRankNum = avgRankNum_;
        if (unlikely(i == totalTaskNum_ - 1)) {
            actualRankNum = tailRankNum_;
